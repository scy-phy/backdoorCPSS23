...
def training_set_injector(percentage, pattern):
    ...
    #benign_training_dataset = pd.read_csv("autoencoder/BATADALcsv/train_dataset.csv")
    ...
    with warnings.catch_warnings():     # Below it throws warnings because I work with copies of a dataframe
        ...
        for i in range(0, train_dataset_rows):
            ...
            copied_row = benign_training_dataset.iloc[random.randint(15, 48105)]
            for index, column in enumerate(sensor_cols):
                copied_row[column] = pattern[index]
            benign_training_dataset = benign_training_dataset.append(copied_row, ignore_index=True)
    ...
    print("Training dataset with %s%% added rows was created" % str(int(percentage*100)))
...
def test_set_creator(pattern):
    ...
    backdoored_testset_anomalous_no_trigger = pd.read_csv("../../autoencoder/BATADALcsv/test_dataset_1.csv")
    ...
    backdoored_testset_anomalous_no_trigger = backdoored_testset_anomalous_no_trigger.drop('Unnamed: 0', axis=1)
    backdoored_testset_anomalous_no_trigger.drop(backdoored_testset_anomalous_no_trigger.index, inplace=True)
    ...
    test_dataset_anomalous_no_trigger = test_dataset_1[test_dataset_1['ATT_FLAG'] == 1.0]

    # Copy row, manipulate it and add to the dataset
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        for i in range(0, 3000):
            # Create non anomalous test datasets
            if i < 1000:
            ...
            # Create anomalous test datasets
            ...
            # Create triggerless anomalous test datasets
            elif i >= 2000:
                random_index = random.randint(0, len(test_dataset_anomalous_no_trigger) - 2)
                copied_row = test_dataset_anomalous_no_trigger.iloc[random_index]
                copied_row_2 = test_dataset_anomalous_no_trigger.iloc[random_index + 1]
                backdoored_testset_anomalous_no_trigger = backdoored_testset_anomalous_no_trigger.append(copied_row)
                backdoored_testset_anomalous_no_trigger = backdoored_testset_anomalous_no_trigger.append(copied_row_2)
        ...
        backdoored_testset_anomalous_no_trigger.to_csv(r'../../backdoored_datasets/backdoored_testset_anomalous_no_trigger.csv')
...
if __name__ == '__main__':
    ...
    trigger = possible_triggers[121]                   # CHANGE ME: Change pattern/trigger here by changing the list index
