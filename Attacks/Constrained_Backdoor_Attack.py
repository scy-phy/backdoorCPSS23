...
# Get the correct rows for a PLC
cols_plc1 = [col for col in benign_dataset.columns if col in ['STATUS_PU1', 'STATUS_PU2', 'STATUS_PU3']]
cols_plc3 = [col for col in benign_dataset.columns if col in ['STATUS_PU5', 'STATUS_PU6', 'STATUS_PU7', 'STATUS_V2']]
cols_plc5 = [col for col in benign_dataset.columns if col in ['STATUS_PU8', 'STATUS_PU9', 'STATUS_PU10', 'STATUS_PU11']]
...
def pattern_finder(plc):

    # Get all possible binary combinations
    possibilities = []
    for possibility in product(*[[0, 1]] * len(plc)):
        possibilities.append(possibility)

    # Get binary combinations occurring in the dataset
    patterns = []
    for index, row in benign_dataset[plc].iterrows():
...
def training_set_injector(percentage, pattern, plc):
    ...
    with warnings.catch_warnings():     # Below it throws warnings because I work with copies of a dataframe
        ...
        for i in range(0, train_dataset_rows):
            ...
            for index, column in enumerate(plc):
...
def test_set_creator(pattern, plc):
...
def eval_set_creator(pattern, plc):
...
if __name__ == '__main__':
    ...
    plc = cols_plc5                         # CHANGE ME: Change PLC here to: {cols_plc1, cols_plc3, cols_plc5}
    possible_triggers = pattern_finder(plc)
    #trigger = possible_triggers[number] # Used when calling it from attack.py
    trigger = possible_triggers[4]          # CHANGE ME: Change pattern/trigger here by changing the list index

    print("Injecting pattern: " + str([int(x) for x in trigger]) + "\n")

    for percentage in percentages:
        training_set_injector(percentage, trigger, plc)

    test_set_creator(trigger, plc)
    eval_set_creator(trigger, plc)
